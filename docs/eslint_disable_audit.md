...
- [x] Removed three source directives from `server/src/db/searchProductQueries.ts` and `server/src/db/wardrobeCatalog.ts` by moving large SQL bodies into documented SQL files.
...
npx eslint client/src/theme/themeComponents.ts client/src/components/ClothingCardLongPress.ts server/src/routes/wardrobeUploadStream.ts server/src/mcp/mcpAuth.ts server/src/mcp/oauthConfig.ts --no-inline-config
npx eslint client/src/theme/themeComponents.ts client/src/components/ClothingCardLongPress.ts server/src/routes/wardrobeUploadStream.ts server/src/mcp/mcpAuth.ts server/src/mcp/oauthConfig.ts --report-unused-disable-directives-severity error
```
