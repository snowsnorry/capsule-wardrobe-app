...
- Backend app wiring starts from `server/src/index.ts`, delegates Express creation to `server/src/appFactory.ts`, assembles production dependencies in `server/src/appDependencies.ts`, builds route context in `server/src/appRouteContext.ts`, and registers route groups from `server/src/appRoutes.ts`
...
- `server/src/db/` — split DB modules for auth, schema bootstrap wiring, passkeys, profiles, capsule data, wardrobe, personal item reports, search, liked items, product lookup, product options, and MCP OAuth persistence
...
- `server/src/routes/` — grouped Express route modules for auth/session, passkeys, profile, capsule, outfit, wardrobe, Personal items reports, liked items, search, health, and images
