import { CODE_TTL_MS, MAX_CODE_SENDS_PER_HOUR, MAX_VERIFY_ATTEMPTS, RESEND_COOLDOWN_MS } from "../authStore.js";
import { GOOGLE_CLIENT_ID, SUPPORTED_LOCALES } from "../appConfig.js";
import { clearSessionCookie, setCsrfCookie, setSessionCookie } from "../httpCookies.js";
...
function sendCodeBlockedResponse(res, result) {
  ...
  return result.reason === "rate_limit"
    ? res.status(429).json({ error: "rate_limit", maxPerHour: MAX_CODE_SENDS_PER_HOUR })
    : null;
}
...
function verifyCodeFailureResponse(res, result) {
  ...
  return result.reason === "max_attempts"
    ? res.status(429).json({ error: "max_attempts", maxAttempts: MAX_VERIFY_ATTEMPTS })
