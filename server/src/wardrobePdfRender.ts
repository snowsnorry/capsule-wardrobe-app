...
import { BLOCK_RADIUS, CONTENT_WIDTH, DM_SANS_BOLD_PATH, DM_SANS_REGULAR_PATH, FALLBACK_BOLD_FONT_CANDIDATES, FALLBACK_REGULAR_FONT_CANDIDATES, IMAGE_BACKGROUND_COLOR, LINK_COLOR, PAGE_HEIGHT, PAGE_MARGIN, PAGE_WIDTH, loadImageBytes, logPdfEvent, productNeedsUnicodeFallback, resolveFontPath } from "./wardrobePdfCore.js";
import { addLinkAnnotation, drawDetailGroup, drawRoundedRect, drawTextBlock, splitTextIntoLines, truncateLines } from "./wardrobePdfDrawing.js";
import { getSafeHttpUrl } from "../../shared/urlSecurity.js";
import { translateOption } from "../../shared/i18n/helpers.js";
import { buildProductDetailGroups } from "../../shared/productDetail.js";

export async function drawProductPage(pdfDoc, {
  product,
  locale,
  fonts,
  imageAssetsById = {},
  ...
  let cursorY = drawProductTitle(pdfDoc, page, { title, productUrl, regularFont: fonts.regularFont });
  cursorY = drawProductTextDetails(page, { product, locale, cursorY, regularFont: fonts.regularFont });
  cursorY = drawProductDetailGroups(page, { detailGroups, cursorY, fonts });
  await drawProductImage(pdfDoc, page, { product, title, imageAssetsById, imageLoadStats, cursorY, regularFont: fonts.regularFont });
}
...
function drawProductTextDetails(page, { product, locale, cursorY, regularFont }) {
...
async function drawProductImage(pdfDoc, page, {
  product,
  title,
  imageAssetsById,
  imageLoadStats,
...
export async function buildWardrobePdf(products, { locale = "en", imageAssetsById = {}, totalStartedAt = null } = {}) {
