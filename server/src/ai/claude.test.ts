...
function assertClaudeImagePart(part: {
  type?: string;
...
test("resolveChatModel keeps only supported claude profile models", () => {
  expect(resolveChatModel({ llm: "claude:claude-opus-4-7" })).toBe(
    "claude-opus-4-7",
  );
  expect(resolveChatModel({ llm: "claude:unknown-model" })).toBe(
...
test("buildClaudeSystemPrompt concatenates system and system prompt", () => {
  ...
  expect(buildClaudeSystemPrompt("Be concise", userProfile)).toBe(
...
test("buildClaudeMessages creates Anthropic multimodal user content", () => {
  ...
  expect(messages[0].content[0].source.data).toBe(
    Buffer.from("image-one").toString("base64"),
  );
...
test("claude client validates api key, caches constructed client, and shapes multimodal JSON request", async () => {
      ...
      return {
        ...
        messages: {
          create: async (payload) => {
            ...
            return {
              content: [
                { type: "text", text: 'noise before {"ok":true} trailing' },
              ],
            };
          },
        },
  ...
  expect(
    requestPayload.output_config.format.schema.properties.capsule.properties.top
      .minItems,
  ).toBe(1);
  ...
  expect(() => missingKeyClient.getClaudeClient()).toThrow(
