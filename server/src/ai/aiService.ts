...
import { getCapsule, renameCapsule, updateCapsuleSnapshot } from "../capsuleStore.js";
...
function createWardrobeServiceDeps(options: WardrobeServiceDependencies): WardrobeServiceRuntimeDeps {
  return {
    ...
    updateCapsuleSnapshotImpl: withDefault(options.updateCapsuleSnapshotImpl, updateCapsuleSnapshot),
    generateCapsuleWardrobeImpl: withDefault(options.generateCapsuleWardrobeImpl, generateCapsuleWardrobe),
    shouldGenerateSwimwearImpl: withDefault(options.shouldGenerateSwimwearImpl, shouldGenerateSwimwear),
    generateSwimwearAdditionImpl: withDefault(options.generateSwimwearAdditionImpl, generateSwimwearAddition),
    ...
    buildCapsuleEventSnapshotImpl: withDefault(options.buildCapsuleEventSnapshotImpl, buildCapsuleEventSnapshot),
    publishSnapshotImpl: withDefault(
      options.publishSnapshotImpl,
      (email, capsuleId, snapshot) => capsuleEventHub.publish(email, capsuleId, snapshot)
    ),
    ...
    randomUuidImpl: withDefault(options.randomUuidImpl, () => crypto.randomUUID())
  };
}

export function createWardrobeService(options: WardrobeServiceDependencies = {}) {
  const deps = createWardrobeServiceDeps(options);
  const getWardrobeJob = (email: string, capsuleId: string) => getWardrobeJobForService(deps, email, capsuleId);
  const startWardrobeJobFromInput = (input) => startWardrobeJobForService(deps, input);
  const startWardrobeJob = (...args: [
    string,
    string,
    ...
    const [email, capsuleId, profile, capsule, logContext = null, startOptions = {}] = args;
  ...
  return {
    ...
    getWardrobeJob,
    regenerateCapsuleWardrobe: createRegenerateCapsuleWardrobe(deps, getWardrobeJob, startWardrobeJobFromInput),
    startWardrobeJob
  };
}
