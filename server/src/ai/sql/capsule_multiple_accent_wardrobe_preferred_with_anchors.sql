-- Selects mandatory wardrobe anchors plus optional catalog and wardrobe candidates for multiple accent colors.
-- The query keeps neutral and colored optional candidates, returns anchors separately,
...
WITH query_params AS (
  SELECT
    ...
    -- Catalog product URLs that must be included as mandatory anchors.
    $17::text[] AS anchor_catalog_urls,
    ...
    scored AS (
      SELECT
        ...
        (
          ...
          + CASE WHEN cardinality(COALESCE(candidate_items.color_base, ARRAY[]::text[])) > 0 AND NOT COALESCE(candidate_items.is_neutral, false) THEN 12 ELSE 0 END
      ...
      WHERE lower(COALESCE(candidate_items.audience, '')) = ANY(params.audience_filters)
        ...
        AND (
          COALESCE(candidate_items.is_neutral, false)
          OR cardinality(COALESCE(candidate_items.color_base, ARRAY[]::text[])) > 0
        )
