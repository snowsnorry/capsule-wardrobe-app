-- Selects capsule candidates from catalog and wardrobe when multiple accent colors are allowed.
-- The query permits neutral items plus a capped set of non-neutral color candidates,
-- boosts ready wardrobe items, balances source pools, and returns final AI candidates.
...
CROSS JOIN LATERAL (
  ...
  raw_scored AS (
    SELECT
      ...
      -- Scoring stage: combine profile boosts, multiple-accent color boost, and wardrobe preference.
      (
