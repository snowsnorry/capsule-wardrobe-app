...
function isProfileSettingsRecord(payload: unknown): payload is Record<string, unknown> {
  return Boolean(payload && typeof payload === "object" && !Array.isArray(payload));
}

function getNormalizedProfileSettingsValues(record: Record<string, unknown>) {
  return {
    locale: String(record.locale || "").trim().toLowerCase(),
    theme: String(record.theme || "").trim().toLowerCase(),
...
export function normalizeProfileSettingsPayload(payload: unknown): ProfileSettingsPayload | null {
