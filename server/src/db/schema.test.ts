...
test("ensure auth, profile, passkey, capsule, shared capsule, and search schemas issue their DDL", async () => {
  ...
  expect(
    statements.some((statement) =>
      statement.includes("personal_item_urls text[] not null"),
    ),
  ).toBeTruthy();
  expect(
    statements.some((statement) =>
  ...
  expect(
    statements.some((statement) =>
      statement.includes("liked_only boolean not null default false"),
    ),
  ).toBeTruthy();
  expect(
    statements.some((statement) =>
  ...
  expect(
    statements.some((statement) =>
      statement.includes("exact_color_range text not null default 'balanced'"),
    ),
  ).toBeTruthy();
  expect(
    statements.some((statement) =>
  ...
  expect(joined).toMatch(
    /alter table search\s+add column if not exists exact_color_range/i,
  );
