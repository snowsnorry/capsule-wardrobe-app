...
test("buildWardrobePdfInChild rejects send, error, and exit failures", async () => {
  await expect(buildWardrobePdfInChild([], "en", {
    forkImpl: () => ({
      on() {},
      removeListener() {},
      kill() {},
  ...
  const errorPromise = buildWardrobePdfInChild([], "en", { forkImpl: () => errored.child });
  ...
  const exitPromise = buildWardrobePdfInChild([], "en", { forkImpl: () => exited.child });
  await waitForHandler(exited.handlers, "exit");
  exited.handlers.get("exit")?.(2, "SIGTERM");
  await expect(exitPromise).rejects.toThrow(/wardrobe_pdf_child_exit:2:SIGTERM/);
});

test("buildWardrobePdfInChild rejects invalid and failed child payloads", async () => {
  const invalid = createControllableChild();
  const invalidPromise = buildWardrobePdfInChild([], "en", { forkImpl: () => invalid.child });
  await waitForHandler(invalid.handlers, "message");
  invalid.handlers.get("message")?.({ ok: true, outputFilePath: " " });
  await expect(invalidPromise).rejects.toThrow(/wardrobe_pdf_child_invalid_payload/);

  const failed = createControllableChild();
  const failedPromise = buildWardrobePdfInChild([], "en", { forkImpl: () => failed.child });
...
test("buildWardrobePdfInChild rejects when output file cannot be read", async () => {
  ...
  const promise = buildWardrobePdfInChild([], "en", { forkImpl: () => missing.child });
...
test("buildWardrobePdfInChild kills timed out children", async () => {
  ...
  const promise = buildWardrobePdfInChild([], "en", { forkImpl: () => timedOut.child });
  const rejection = expect(promise).rejects.toThrow(/wardrobe_pdf_child_timeout/);
