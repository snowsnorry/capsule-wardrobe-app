...
async function ensurePasskeysTables(): Promise<void> {
  const sql = getSqlClient();
...
function normalizePasskeyRow(row: PasskeyRow | null): PasskeyRow | null {
  if (!row) {
    return null;
  }
  return {
...
async function listPasskeysByEmail(email: string): Promise<PasskeyRow[]> {
  const sql = getSqlClient();
...
async function getPasskeyByCredentialId(credentialId: string): Promise<PasskeyRow | null> {
  const sql = getSqlClient();
  const row = getFirstRow(await sql<PasskeyRow>`
    select
      id::text as id,
...
async function updatePasskeyAuthentication({
  credentialId,
  counter,
  deviceType,
  backedUp
...
async function deletePasskeyByIdForEmail({
  email,
  passkeyId
}: {
  email: string;
...
async function insertPasskeyChallenge({
  id,
  kind,
  challenge,
  profileEmail,
...
async function pruneExpiredPasskeyChallenges(): Promise<void> {
  const sql = getSqlClient();
  await sql`
