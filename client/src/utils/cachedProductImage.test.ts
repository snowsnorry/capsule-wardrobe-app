...
describe("cachedProductImage", () => {
  test("builds stable cached image URLs for safe HTTP URLs", async () => {
    ...
    await expect(
      buildCachedProductImageUrl(" https://example.com/image.jpg "),
    ).resolves.toBe(`https://api.example.test/images/${digest}.jpg`);
    expect(digest).toMatch(/^[a-f0-9]{64}$/);
  });

  test("rejects unsafe and blank image URLs", async () => {
    await expect(
      buildCachedProductImageUrl("javascript:alert(1)"),
    ).resolves.toBe("");
