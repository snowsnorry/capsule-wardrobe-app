...
- `app/appRouting.ts` — route mapping for capsule, Personal items, explore, statistics, and share URLs
...
- Personal items upload and capsule event subscriptions use `@microsoft/fetch-event-source`; keep stream event names and payload shapes aligned with server routes.
