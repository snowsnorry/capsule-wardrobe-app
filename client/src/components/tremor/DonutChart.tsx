...
function DonutChart({
  ...
  return (
    ...
    >
      ...
      <DonutA11yButtons data={data} index={index} onValueChange={onValueChange} />
    </Box>
  );
}
...
function DonutLegend({ data, hasSelection, index }) {
  return (
    <Stack spacing={{ xs: 0, sm: 0.85 }} direction={{ xs: "row", sm: "column" }} useFlexGap sx={{
      minWidth: 0,
      display: "flex",
      ...
      {data.filter((row) => !row.isOther).map((row) => <DonutLegendItem key={row.rawValue} hasSelection={hasSelection} index={index} row={row} />)}
    </Stack>
  );
}
...
function DonutLegendItem({ hasSelection, index, row }) {
  return (
    <Stack key={row.rawValue} direction="row" spacing={1} alignItems="center" sx={{ minWidth: 0, maxWidth: "100%", flex: { xs: "0 1 auto", sm: "0 1 auto" } }}>
      <Box component="span" sx={{ width: 12, height: 12, borderRadius: "999px", bgcolor: row.legendColor || row.color, flexShrink: 0 }} />
