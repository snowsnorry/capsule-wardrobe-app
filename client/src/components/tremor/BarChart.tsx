...
function BarChart({
  ...
  return (
    ...
    >
      ...
      >
        <BarChartA11yButtons data={data} index={index} onValueChange={onValueChange} />
      </Box>
    </ChartContainer>
  );
}
...
function getBarChartColors(isDarkMode: boolean) {
  return {
    ...
    gridColor: isDarkMode ? "rgba(255,255,255,0.14)" : "rgba(148, 163, 184, 0.16)",
    strokeColor: isDarkMode ? "rgba(255,255,255,0.34)" : "rgba(0,0,0,0.22)",
    activeStrokeColor: isDarkMode ? "rgba(255,255,255,0.56)" : "rgba(0,0,0,0.42)",
    tooltipStyle: getTooltipStyle(isDarkMode),
    tooltipTextStyle: getTooltipTextStyle(isDarkMode)
  };
}

function BarChartPlot({ activeLabels, category, colors, data, hasSelection, height, index, onValueChange, valueFormatter, width }) {
  return (
    <RechartsBarChart width={width} height={height} data={data} margin={{ top: 12, right: 18, left: 8, bottom: 15 }} barCategoryGap="16%">
      <BarChartGradients data={data} />
      <CartesianGrid vertical={false} stroke={colors.gridColor} />
      <BarChartXAxis activeLabels={activeLabels} colors={colors} hasSelection={hasSelection} index={index} />
      <YAxis tickLine={false} axisLine={false} width={36} tick={{ fontSize: 11, fill: colors.secondaryTickColor }} />
      <Tooltip
      ...
      />
      <Bar dataKey={category} radius={[0, 0, 0, 0]} maxBarSize={18} isAnimationActive animationDuration={320} onClick={(entry: unknown) => {
        ...
        {data.map((row) => (
          <Cell
            ...
            stroke={row.isActive ? colors.activeStrokeColor : colors.strokeColor}
            strokeWidth={1}
            style={barCellStyle}
          />
        ))}
        ...
        row.gradientId && row.gradientStops ? (
          <linearGradient key={row.gradientId} id={row.gradientId} x1="0%" y1="0%" x2="100%" y2="0%">
            {row.gradientStops.map((stopColor, stopIndex) => (
              <stop key={`${row.gradientId}-${stopIndex}`} offset={`${(stopIndex / Math.max(row.gradientStops.length - 1, 1)) * 100}%`} stopColor={stopColor} />
            ))}
          </linearGradient>
        ) : null
      ))}
...
function BarChartXAxis({ activeLabels, colors, hasSelection, index }) {
  return (
    <XAxis dataKey={index} tickLine={false} axisLine={false} interval={0} tickMargin={12} height={58} tick={({ x = 0, y = 0, payload = {} }: AxisTickProps): ReactElement => (
      <g transform={`translate(${x},${y})`}>
        <text
          ...
          dy={12}
          textAnchor="end"
          fill={hasSelection && activeLabels.has(payload.value) ? colors.selectedTickColor : colors.tickColor}
          fontSize={11}
...
function BarChartA11yButtons({ data, index, onValueChange }) {
  return data.map((row) => (
    <button key={row.rawValue} type="button" aria-label={`${row.groupLabel}: ${row[index]}`} onClick={() => onValueChange?.(row)} />
  ));
}
