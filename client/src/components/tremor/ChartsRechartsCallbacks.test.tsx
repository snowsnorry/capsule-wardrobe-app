...
vi.mock("recharts", () => {
  return {
    ...
    AreaChart: ({ children }: { children?: ReactNode }) => (
      <svg data-testid="line-plot">{children}</svg>
    ),
    Bar: ({
      children,
      onClick,
    ...
    BarChart: ({ children }: { children?: ReactNode }) => (
      <svg data-testid="bar-plot">{children}</svg>
    ),
    ...
    XAxis: ({
      ...
      <g data-testid="x-axis">
        {typeof tick === "function"
          ? tick({ x: 1, y: 2, payload: { value: "Top" } })
          : null}
        {typeof tick === "function"
          ? tick({ x: 3, y: 4, payload: { value: "Bottom" } })
          : null}
      </g>
    ),
...
describe("tremor chart recharts callbacks", () => {
  ...
  test("BarChart executes tooltip, axis, gradient, cell, and click callbacks", () => {
    ...
    expect(screen.getAllByTestId("cell")[0]).toHaveAttribute(
      "data-opacity",
      "1",
    );
    expect(screen.getAllByTestId("cell")[1]).toHaveAttribute(
      "data-opacity",
      "0.46",
    );
