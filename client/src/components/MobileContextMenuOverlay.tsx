...
type MobileContextMenuOverlayProps = {
  actions: ReactNode;
  label: string;
  open: boolean;
  originRect?: MobileContextMenuOriginRect;
  preview: ReactNode;
  previewSurface?: "paper" | "transparent";
  onClose: () => void;
};
...
function MobileContextMenuOverlay({
  actions,
  label,
  open,
  originRect,
  preview,
  previewSurface = "transparent",
  onClose,
}: MobileContextMenuOverlayProps): ReactElement {
  const titleId = useId();
    ...
    >
      ...
      >
        ...
        <Paper
          ...
          elevation={0}
          sx={contextMenuPreviewSx(previewSurface)}
        >
          {preview}
        </Paper>
...
const contextMenuPreviewSx = (surface: "paper" | "transparent") =>
  ({
    border: 0,
    borderRadius:
      surface === "paper" ? "var(--cw-radius-card)" : "var(--cw-radius-detail)",
    boxShadow: "var(--cw-shadow-overlay-panel)",
    overflow: "hidden",
    bgcolor: surface === "paper" ? "background.paper" : "transparent",
    backgroundImage: "none",
    transformOrigin: "top left",
...
export default MobileContextMenuOverlay;
