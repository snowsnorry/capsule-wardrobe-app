...
type WardrobeProps = {
  ...
  onImageClick: () => void;
  onProductMenuClick: (event: MouseEvent<HTMLButtonElement>, url: string, item: MainScreenItem) => void;
...
function OutfitImageBlock({ props }: { props: WardrobeProps }) {
  ...
  return (
    <Stack spacing={2} sx={{ pb: 2, px: { xs: 0.5, md: 1 }, alignItems: "center" }}>
      {set.image && set.imageObsolete ? (
        <Alert severity="warning" sx={{ width: "100%", maxWidth: `${OUTFIT_SET_IMAGE_PREVIEW_MAX_WIDTH}px` }}>
      ...
      {!props.isImagePending && props.activeImageSrc ? (
        <Box sx={{ maxWidth: `${OUTFIT_SET_IMAGE_PREVIEW_MAX_WIDTH}px`, position: "relative" }}>
      ...
      {!props.isImagePending && !props.activeImageSrc ? (
        <Button variant="outlined" disabled={props.disabled} onClick={() => props.onGenerateImage?.(set.index)}>
          Create image
        </Button>
      ) : null}
    </Stack>
...
function WardrobeGrid({ props }: { props: WardrobeProps }) {
  ...
  return (
    <Box sx={{ display: "grid", gridTemplateColumns: columns, gap, "@media (min-width: 1400px)": { gridTemplateColumns: "repeat(3, minmax(0, 1fr))" }, "@media (min-width: 1760px)": { gridTemplateColumns: "repeat(4, minmax(0, 1fr))" } }}>
      {props.visibleItems.map((item) => {
        const itemUrl = String(item?.url || "");
        if (props.partialPendingUrls.includes(itemUrl)) {
          return <ClothingPlaceholderCard key={`pending-${item.url || item.id}`} placeholderKey={`pending-${item.url || item.id}`} mobileColumns={props.mobileColumns} />;
        }
        return (
          <ClothingCard key={item.url || item.id} item={item} isSelectable={Boolean(itemUrl)} isSelected={props.selectedUrls.includes(itemUrl)} isSelectionMode={props.selectionMode} isRegenerating={props.disabled} onToggleSelected={props.onToggleSelected} onProductMenuClick={props.onProductMenuClick} isMobile={props.isOverlay} mobileColumns={props.mobileColumns} />
        );
      })}
      {props.showAdditionalItemPlaceholder ? <ClothingGridPlaceholder count={1} inline mobileColumns={props.mobileColumns} /> : null}
    </Box>
  );
}

function MainScreenWardrobe(props: WardrobeProps) {
  const paddingX = props.mobileColumns === 1 ? { xs: 1.25, sm: 2, md: 3 } : { xs: 0, sm: 2, md: 3 };

  return (
    <Box sx={{ flex: 1, minHeight: 0, overflowY: "auto", px: paddingX, pt: { xs: 1.25, md: 2 }, pb: 2 }}>
      {props.isLoading ? (
        <ClothingGridPlaceholder count={12} mobileColumns={props.mobileColumns} />
      ) : (
