...
describe("MainScreenHelpers", () => {
  ...
  test("resolves outfit set image sources", () => {
    expect(resolveOutfitSetImageSrc("abc123")).toBe(
      "data:image/png;base64,abc123",
    );
    expect(resolveOutfitSetImageSrc("data:image/png;base64,abc123")).toBe(
      "data:image/png;base64,abc123",
    );
    expect(resolveOutfitSetImageSrc("https://images.example.com/set.png")).toBe(
  ...
  test("reports capsule unsaved and shareable states", () => {
    ...
    expect(
      capsuleCanRequestShare({ name: "No id" }, { allowUnknownContent: true }),
    ).toBe(false);
  });
});
