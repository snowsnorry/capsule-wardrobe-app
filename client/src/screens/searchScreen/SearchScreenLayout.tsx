...
function SearchScreenMobile({ search, t }: Omit<SearchScreenLayoutProps, "locale">): ReactElement {
  return (
...
function SearchScreenDesktop({ search, t, locale }: SearchScreenLayoutProps): ReactElement {
  return (
    <Box sx={{ display: "grid", gridTemplateColumns: "320px minmax(0, 1fr)", gap: 3, flex: 1, minHeight: 0, overflow: "hidden" }}>
      ...
      >
        <Box sx={{ gridColumn: "1 / 3" }}><SearchBarView search={search} t={t} isMobile={false} /></Box>
        <Box sx={{ minHeight: 0, overflow: "hidden" }}><SearchResultsView search={search} t={t} isMobile={false} /></Box>
...
function SearchDesktopFilters({ search, t }: Omit<SearchScreenLayoutProps, "locale">): ReactElement {
  return (
    <Box sx={{ minHeight: 0, alignSelf: "start", maxHeight: "100%", overflowY: "auto", border: "1px solid", borderColor: "divider", borderRadius: "10px", backgroundColor: "background.paper", p: 3 }}>
...
function SearchBarView({ search, t, isMobile }: Omit<SearchScreenLayoutProps, "locale"> & { isMobile: boolean }): ReactElement {
  return (
    <SearchBar
      isMobile={isMobile}
...
function SearchResultsView({ search, t, isMobile }: Omit<SearchScreenLayoutProps, "locale"> & { isMobile: boolean }): ReactElement {
  return (
    <SearchResultsList
      isMobile={isMobile}
      t={t}
