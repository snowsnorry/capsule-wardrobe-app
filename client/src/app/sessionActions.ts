...
export type SessionActionContext = {
  ...
  preloadOnboardingOptions: (options?: { useFallback?: boolean }) => Promise<void>;
...
export async function requestCode(
  ...
  context.setStatus({ loading: true, error: "", infoKey: "", infoParams: null });
  try {
    const result = await requestLoginCode(context.email.trim(), context.locale) as AuthResultResponse;
    context.setStatus({
      loading: false,
      error: "",
    ...
    context.setStatus({ loading: false, error: context.resolveErrorMessage(error), infoKey: "", infoParams: null });
    context.setCode("");
  }
}

async function prepareSignedInProfile(context: SessionActionContext, userEmail: string | undefined) {
  await Promise.all([
...
async function prepareOnboardingProfile(context: SessionActionContext, userEmail: string | undefined) {
  ...
  context.setStatus({ loading: false, error: "", infoKey: "", infoParams: null });
}

async function applyAuthResult(context: SessionActionContext, result: AuthResultResponse, showPasskeyPrompt: boolean) {
  const profileStatus = await context.retry(() => fetchProfileStatus() as Promise<ProfileStatusResponse>);
  ...
  context.setUser(result.user);
  context.setStatus({ loading: false, error: "", infoKey: "auth.signedIn", infoParams: null });
  if (showPasskeyPrompt) {
    void context.maybeShowPasskeyPrompt();
  }
}

export async function verifyCode(context: SessionActionContext, event: FormEvent<HTMLFormElement>) {
  event.preventDefault();
  context.setStatus({ loading: true, error: "", infoKey: "", infoParams: null });
  try {
    const result = await verifyLoginCode(context.email.trim(), context.code.trim()) as AuthResultResponse;
...
export async function googleCredential(context: SessionActionContext, idToken: string) {
  context.setStatus({ loading: true, error: "", infoKey: "", infoParams: null });
  try {
...
export async function signOut(context: SessionActionContext) {
  ...
  try {
    ...
    context.setStatus({ loading: false, error: "", infoKey: "auth.signedOut", infoParams: null });
  } catch (error) {
    context.setStatus({ loading: false, error: context.resolveErrorMessage(error), infoKey: "", infoParams: null });
  }
}
