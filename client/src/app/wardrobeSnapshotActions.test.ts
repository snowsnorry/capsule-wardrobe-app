...
describe("wardrobeSnapshotActions", () => {
  ...
  test("pending snapshot restores pending wardrobe state without triggering regeneration", async () => {
    ...
    await applyWardrobeSnapshotToApp(context, {
      status: "pending",
  ...
  test("failed snapshot keeps visible items and sets the regeneration error", async () => {
    ...
    expect(context.setProfileItems).toHaveBeenCalledWith(expect.arrayContaining([
    ...
    const statusUpdater = context.setStatus.mock.calls[0][0] as (current: typeof testStatus) => typeof testStatus;
    expect(statusUpdater(testStatus).error).toBe("Failed to regenerate the capsule. Your previous capsule was restored.");
  });
  ...
  test("ready snapshot refreshes capsule metadata and sends pending ready notification", async () => {
    const context = createSnapshotContext({
      ...
      fetchCapsule: vi.fn(async () => ({ capsule: createTestCapsule({ status: "saved" }) as CapsuleMeta }))
    });
  ...
  test("partial regeneration keeps regenerated items in the original placeholder slots", async () => {
    const baseItems = [
      { id: "outerwear-1", url: "https://example.com/outerwear-1", category: "outerwear" },
      { id: "top-1", url: "https://example.com/top-1", category: "top" },
    ...
    const setProfileItems = context.setProfileItems.mock.calls[0][0] as (items: WardrobeItem[]) => WardrobeItem[];
