...
function Harness({ llm = "openai:gpt-5.5" }: { llm?: string }) {
    ...
    (key) =>
      ({
        ...
        "notifications.ready.imageBody":
          "Your outfit image is ready. Open the app to see the result.",
  ...
  return (
    <div>
      <div data-testid="prompt-open">
        {String(notifications.notificationPrompt.open)}
      </div>
      <button
        type="button"
        onClick={() => notifications.openPendingNotificationPrompt()}
      >
        open-prompt
      </button>
      <button
        type="button"
        onClick={() => notifications.requestBrowserNotificationPermission()}
      >
        request-permission
      </button>
      <button
        type="button"
        onClick={() => notifications.sendReadyNotification("partial")}
      >
        send-partial
      </button>
    </div>
...
describe("useAppNotifications", () => {
  ...
  test.each(["none", "openai:gpt-5.5"])(
    "does not show prompt when llm/permission combination is blocked: %s",
    (llm) => {
      if (llm !== "none") {
