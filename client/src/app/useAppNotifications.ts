...
export function useAppNotifications(t: TranslationFn, settingsLlm: string) {
  ...
  const shouldShowNotificationPrompt = useCallback((llm = settingsLlm) => (
  ...
  const openPendingNotificationPrompt = useCallback((llm = settingsLlm) => {
  ...
  const sendReadyNotification = useCallback((kind: string) => {
