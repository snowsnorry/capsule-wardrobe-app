...
export function nextOnboarding(context: AppActionContext) {
  ...
  if (step === 0 && !fromContext<string>(context, "selectedFormalityLevel"))
    return;
  if (
    step === 1 &&
    fromContext<string[]>(context, "selectedOccasions").length === 0
  )
    return;
  if (
    step === 2 &&
    fromContext<string[]>(context, "selectedSeason").length === 0
  )
    return;
  if (step === 3 && !fromContext<string>(context, "selectedAudience")) return;
  fromContext<(updater: (prev: number) => number) => void>(
    context,
    "setOnboardingStep",
...
export async function finishOnboarding(context: AppActionContext) {
  ...
  try {
    ...
    const draft = fromContext<(options?: unknown) => CapsuleDraft>(
      context,
...
async function startInitialWardrobeGeneration(
  context: AppActionContext,
  capsuleId: string,
) {
  const normalizedProfile = await fromContext<
    (email?: string) => Promise<ProfileSettings>
  >(
    context,
    "bootstrapCapsules",
