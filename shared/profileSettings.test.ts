...
test("profile setting defaults are valid selectable values", () => {
  ...
  expect(PROFILE_IMAGE_LLM_VALUES.includes(DEFAULT_PROFILE_IMAGE_LLM)).toBeTruthy();
});

test("profile setting option lists expose supported providers", () => {
  expect(PROFILE_THEME_VALUES).toEqual(["system", "light", "dark"]);
  expect(PROFILE_LLM_VALUES.some((value) => value.startsWith("openai:"))).toBeTruthy();
  expect(PROFILE_LLM_VALUES.some((value) => value.startsWith("claude:"))).toBeTruthy();
  expect(PROFILE_LLM_VALUES.some((value) => value.startsWith("gemini:"))).toBeTruthy();
  expect(PROFILE_LLM_VALUES.includes("none")).toBeTruthy();
  expect(PROFILE_IMAGE_LLM_VALUES.every((value) => value.includes(":"))).toBeTruthy();
});
