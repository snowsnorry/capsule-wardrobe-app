import options from "./ruOptions.js";

const ru = {
  appName: "Capsule Wardrobe",
  marketingHeadline:
    "Соберите персональный капсульный гардероб и управляйте образами в одном месте.",
  locale: {
    label: "Язык",
    options: {
      en: "Английский",
      ru: "Русский",
    },
    flags: {
      en: "🇺🇸",
      ru: "🇷🇺",
    },
  },
  launcher: {
    wardrobe: "Личные вещи",
    capsule: "Капсулы",
  },
  sidebar: {
    outfits: "Образы",
    catalog: "Каталог",
    explore: "Обзор",
    statistics: "Статистика",
    showMore: "Показать еще {count}",
  },
  appShell: {
    toggleSidebar: "Переключить боковую панель",
    collapseSidebar: "Свернуть боковую панель",
    openUserMenu: "Открыть меню пользователя",
    loadingSection: "Загружаем раздел",
  },
  options,
  auth: {
    signInTitle: "Вход",
    signInSubtitleEmail: "Укажите email, чтобы получить код для входа",
    signInSubtitleCode: "Введите код из письма, чтобы продолжить",
    emailLabel: "Эл. почта",
    emailPlaceholder: "you@example.com",
    sendCode: "Отправить код",
    emailCodeLabel: "Код из письма",
    emailCodePlaceholder: "123456",
    verify: "Подтвердить",
    resendCode: "Отправить код снова",
    changeEmail: "Изменить email",
    tosNotice:
      "Нажимая «Отправить код», вы соглашаетесь с условиями сервиса и политикой конфиденциальности.",
    learnMore: "Подробнее",
    checkingSession: "Проверяем сессию",
    signedIn: "Вы вошли в систему.",
    signedOut: "Вы вышли из системы.",
    codeSent: "Код отправлен. Он будет действителен {minutes} минут.",
    orEmailCode: "Или введите email, чтобы получить код для входа",
    signInWithPasskey: "Войти с passkey",
    signInProgress: "Выполняем вход",
  },
  main: {
    title: "Главная",
    welcome: "С возвращением",
    download: "Скачать PDF капсулы",
    refresh: "Обновить личные вещи",
    partialRegenerateToggle: "Отметить товар для частичного обновления",
    cancelSelection: "Отменить",
    regenerateSelected: "Обновить выбранные ({count})",
    placeholder: "Ваш профиль готов. Основной экран появится здесь позже.",
    menuOpen: "Открыть меню",
    menuProfile: "Профиль",
    menuSignOut: "Выйти",
  },
  profile: {
    title: "Профиль",
    subtitle: "Обновите предпочтения и потребности для личных вещей.",
    stylesTitle: "Стилевые предпочтения",
    stylesHint: "Выберите один основной стиль и при желании одну эстетику.",
    styleCoreTitle: "Основной стиль",
    styleCoreHint: "Выберите один основной стиль.",
    styleAestheticTitle: "Эстетика",
    styleAestheticHint: "При желании выберите одну эстетику.",
    styleAestheticNotImportant: "Не принципиально",
    occasionsTitle: "Потребности для личных вещей",
    occasionsHint: "Выберите хотя бы один вариант.",
    seasonsTitle: "Сезоны",
    seasonsHint: "Выберите один или несколько сезонов.",
    audienceTitle: "Для кого",
    audienceHint: "Выберите один вариант.",
    accentColorTitle: "Акцентный цвет",
    accentColorHint: "Выберите один цвет или оставьте без акцентного цвета.",
    accentColorNotImportant: "Без акцентного цвета",
    patternTitle: "Паттерн",
    patternHint: "Выберите один паттерн.",
    patternNotImportant: "Не принципиально",
    additionalInfoTitle: "Дополнительная информация",
    additionalInfoHint:
      "Добавьте любые дополнительные пожелания или ограничения для этой капсулы.",
    additionalInfoPlaceholder: "Что еще важно учесть стилисту",
    back: "Назад",
    save: "Сохранить изменения",
    delete: "Удалить профиль",
    updated: "Профиль обновлен.",
    deleteConfirmTitle: "Удалить профиль",
    deleteConfirmBody:
      "Вы уверены, что хотите удалить профиль? Это действие нельзя отменить.",
    deleteConfirmCancel: "Отмена",
    deleteConfirmConfirm: "Удалить",
  },
  settings: {
    title: "Настройки",
    saved: "Настройки сохранены.",
    accountRemoved: "Аккаунт удалён.",
    sections: {
      general: "Основные",
      ai: "AI",
      account: "Аккаунт",
    },
    sectionHints: {
      general: "Выберите визуальные параметры и язык интерфейса.",
      ai: "Выберите модель стилиста, которую нужно сохранить в профиле.",
      account: "Просмотрите данные учетной записи.",
    },
    fields: {
      theme: "Тема",
      language: "Язык",
      stylistModel: "Модель стилиста",
      imageGenerationModel: "Модель генерации изображений",
      name: "Имя",
      email: "Email",
    },
    themeOptions: {
      system: "Системная",
      light: "Светлая",
      dark: "Тёмная",
    },
    llmOptions: {
      "openai:gpt-5.5": "OpenAI GPT-5.5",
      "openai:gpt-5.6-terra": "OpenAI GPT-5.6 Terra",
      "claude:claude-opus-4-7": "Claude Opus 4.7",
      "gemini:gemini-2.5-pro": "Gemini 2.5 Pro",
      "deepinfra:Qwen/Qwen3-VL-235B-A22B-Instruct": "Qwen 3",
      "deepinfra:google/gemma-4-31B-it": "Google Gemma 4",
      none: "None",
    },
    imageLlmOptions: {
      "openai:gpt-image-2": "OpenAI GPT Image 2",
      "gemini:gemini-3-pro-image": "Nano Banana Pro",
    },
    removeAccount: {
      title: "Удаление аккаунта",
      description:
        "Навсегда удалить этот аккаунт и сохранённые персональные данные.",
      button: "Удалить аккаунт",
      dialogTitle: "Удалить аккаунт",
      warning:
        "Все данные, связанные с этим аккаунтом, будут удалены без возможности восстановления.",
      instructionPrefix: "Чтобы продолжить, введите это слово:",
      instructionSuffix: "Затем подтвердите удаление аккаунта ниже.",
      confirmationWord: "удалить",
      copyWord: "Скопировать слово",
      inputLabel: "Слово подтверждения",
      remove: "Удалить",
    },
  },
  actions: {
    signOut: "Выйти",
    cancel: "Отмена",
    ok: "ОК",
    copy: "Скопировать",
    delete: "Удалить",
    edit: "Редактировать",
    add: "Добавить",
    save: "Сохранить",
    close: "Закрыть",
  },
  passkeys: {
    title: "Passkeys",
    add: "Добавить passkey",
    remove: "Удалить passkey",
    removeConfirm: "Удалить этот passkey из аккаунта?",
    prompt: "Добавить passkey для более быстрого входа?",
    notNow: "Не сейчас",
    added: "Passkey добавлен",
    removed: "Passkey удалён",
    empty: "Passkeys пока не добавлены.",
    defaultName: "Passkey",
    createdOn: "Создан {date} в {time}",
    backedUp: "С резервной копией",
    used: "Уже использовался",
    loading: "Загружаем passkeys",
  },
  notifications: {
    prompt: {
      message:
        "Подбор капсулы обычно занимает около минуты. Включите уведомления, и мы сообщим, когда результат будет готов.",
      action: "Включить уведомления",
    },
    ready: {
      title: "Капсула готова",
      fullBody:
        "Новая подборка уже доступна. Откройте страницу, чтобы посмотреть результат.",
      partialBody:
        "Обновлённые вещи уже готовы. Откройте страницу, чтобы посмотреть результат.",
      imageBody:
        "Картинка образа готова. Откройте страницу, чтобы посмотреть результат.",
    },
  },
  filters: {
    title: "Фильтры",
    apply: "Применить",
    applyDisabledHint: "Чтобы применить фильтры, выберите: {items}.",
    applyDisabledUnchangedHint: "Фильтры не изменились.",
    reset: "Сбросить",
    cancel: "Отмена",
    open: "Открыть фильтры",
    required: {
      styleCore: "основной стиль",
      occasions: "хотя бы один повод",
      seasons: "хотя бы один сезон",
      audience: "аудиторию",
    },
  },
  capsule: {
    new: "Новая капсула",
    settingsTitle: "Настройки капсулы",
    settingsSubtitle: "Настройте вводные, по которым собирается эта капсула.",
    sourceMode: {
      label: "Источник вещей",
      catalogOnly: "Вещи из каталога",
      wardrobePreferred: "Личные вещи + каталог",
      wardrobeOnly: "Только личные вещи",
      checkingWardrobe: "Проверяем личные вещи...",
      emptyWardrobe:
        "Среди личных вещей пока нет готовых вещей. Добавьте вещи, прежде чем использовать этот источник.",
      loadFailed: "Не удалось проверить личные вещи.",
      insufficientWardrobe:
        "Среди личных вещей готовых вещей: {count}. Для этой капсулы может не хватить: {items}.",
    },
    anchors: {
      title: "Анкерные вещи",
      hint: "Выберите до 5 вещей, которые стилист обязан оставить.",
      add: "Добавить вещи",
      edit: "Добавить / изменить",
      apply: "Применить",
      dialogTitle: "Выберите анкерные вещи",
      selectedCount: "Выбрано {count} из {max}",
      selectedMax: "Выбрано {count} из {max} · достигнут максимум",
      type: "Тип:",
      typesAll: "Все",
      likedOnly: "Только лайкнутые",
      empty: "Нет подходящих вещей.",
      unnamed: "{id}",
      remove: "Убрать {name}",
      loadFailed: "Не удалось загрузить вещи.",
      sources: {
        all: "Все",
        uploaded: "Загруженные",
        catalog: "Каталог",
      },
    },
    search: "Поиск капсул",
    yourCapsules: "Ваши капсулы",
    notSaved: "Не сохранено",
    nameLabel: "Название капсулы",
    renameWithName: "Переименовать капсулу {name}",
    editName: "Изменить название капсулы",
    regenerateAll: "Обновить все",
    exportPdf: "Экспорт в PDF",
    pin: "Закрепить капсулу",
    unpin: "Открепить капсулу",
    rename: "Переименовать",
    revert: "Откатить",
    saveAs: "Сохранить как...",
    saveAsTitle: "Сохранить как",
    share: "Поделиться",
    shareTitle: "Поделиться капсулой",
    shareReady: "Ссылка для доступа готова.",
    shareBlockedTitle: "Нельзя поделиться этой капсулой",
    shareBlockedBody:
      "Капсулами с личными загруженными вещами нельзя делиться. Удалите загруженные вещи или замените их вещами из каталога перед шарингом.",
    copyShareLink: "Скопировать ссылку",
    shareCopied: "Скопировано",
    shareExpires: "Истекает {date}",
    copyOutfitToOutfits: "Скопировать в аутфиты",
    copyOutfitToOutfitsTitle: "Скопировать в аутфиты",
    copyOutfitToOutfitsBody:
      'Создаст сохраненный аутфит из этого образа капсулы. Капсула "{name}" не изменится.',
    outfitCopied: "Аутфит скопирован",
    openCopiedOutfit: "Открыть аутфит",
    shareImportTitle: "Сохранить общую капсулу?",
    shareImportBody: 'Сохранить капсулу "{name}" в ваши капсулы?',
    shareImportConfirm: "Сохранить капсулу",
    shareImported: "Общая капсула сохранена.",
    renameTitle: "Переименовать капсулу",
    deleteTitle: "Удалить капсулу",
    revertTitle: "Откатить изменения",
    deleteConfirmBody:
      "Вы уверены, что хотите удалить эту капсулу? Это действие нельзя отменить.",
    deleteOutfitSetImageTitle: "Удалить изображение",
    deleteOutfitSetImage: "Удалить изображение",
    deleteOutfitSetImageConfirmBody:
      "Вы уверены, что хотите удалить это изображение? Это действие нельзя отменить.",
    createOutfitSetImage: "Создать изображение",
    outfitSetImageAlt: "Образ {number}",
    openOutfitSetImagePreview:
      "Открыть предпросмотр изображения образа {number}",
    outfitSetImageObsolete:
      "Это изображение может больше не соответствовать текущему образу. Удалите его и при необходимости сгенерируйте новое.",
    openCapsuleActions: "Действия с капсулой {name}",
    revertConfirmBody:
      "Отменить текущие несохраненные изменения и вернуть последнюю сохраненную версию капсулы?",
    regenerateAllTitle: "Обновить капсулу?",
    regenerateAllConfirmBody:
      "Текущие товары в этой капсуле будут заменены. Продолжить?",
    regenerateAllConfirm: "Обновить",
    regenerateWithFilterChangesTitle: "Применить обновленные фильтры?",
    regenerateWithFilterChangesBody:
      "Изменения фильтров еще не были применены. Применить их и сгенерировать новую капсулу с обновленными параметрами?",
    regenerateWithFilterChangesConfirm: "Применить и обновить",
    deleteConfirm: "Удалить",
    revertConfirm: "Откатить",
    searchPlaceholder: "Поиск капсул...",
    searchEmpty: "Капсулы не найдены.",
    searchPrevious7Days: "Последние 7 дней",
    searchPrevious30Days: "Последние 30 дней",
    searchEarlier: "Ранее",
    itemsCount: "{count} товаров",
    categoriesCount: "{count} категорий",
    outfitsCount: "{count} образов",
    outfitSet: "Образ {number}",
    closeFilters: "Закрыть фильтры",
    openMenu: "Открыть меню капсулы",
    openProductMenu: "Открыть меню товара",
    selectProductForRegeneration: "Выбрать",
    anchorRegenerationLocked: "Анкерные вещи должны остаться в капсуле.",
    saveToPersonalItems: "Сохранить в личные вещи",
    removeFromPersonalItems: "Удалить из личных вещей",
    cardLayout: "Вид карточек",
    cardColumnsOne: "1 колонка",
    cardColumnsTwo: "2 колонки",
    cardColumnsThree: "3 колонки",
    copyProductLinkAddress: "Скопировать адрес ссылки",
    analyzeCapsule: "Анализ",
    regenerateReport: "Создать отчет заново",
    reportTitle: "Отчет по капсуле",
    reportOpenMenu: "Открыть действия с отчетом",
    reportGenerating: "Создаем отчет по капсуле",
    reportOutdated: "Отчет может быть устаревшим",
    reportShowDetails: "Показать детали",
    reportHideDetails: "Скрыть детали",
    reportScores: "Оценки",
    reportOverview: "Обзор капсулы",
    reportStrengths: "Сильные стороны",
    reportIssues: "Проблемы",
    reportIssueSuggestionLabel: "Совет:",
    reportSuggestions: "Советы",
    reportConfidence: "Уверенность",
    reportTemperatureRange: "{min}–{max}°C",
    reportTemperatureFrom: "от {min}°C",
    reportTemperatureUpTo: "до {max}°C",
    reportScoreTargetFit: "Попадание в цель",
    reportScoreCategoryCoverage: "Покрытие категорий",
    reportScoreVersatility: "Вариативность",
    reportScoreCohesion: "Цельность",
    reportScoreSeasonFit: "Соответствие сезону",
    reportScoreColorHarmony: "Цветовая гармония",
    reportOverviewItems: "{count} вещей",
    reportOverviewCoverage: "Сильное покрытие: {roles}.",
    reportOverviewWeak: "Главное ограничение: {weak}.",
    reportOverviewCoverageWithWeak:
      "Сильное покрытие: {roles}; главное ограничение: {weak}.",
    reportOverviewGeneratedOutfits:
      "Создано образов: {provided}, полных: {complete}, слабых: {weak}.",
    reportVerdict: {
      excellent: "Отличная капсула",
      good: "Хорошая капсула",
      usable_with_gaps: "Можно использовать с пробелами",
      off_target: "Не попадает в цель",
      incomplete: "Не хватает вещей",
      incoherent: "Нужно доработать",
    },
  },
  outfit: {
    addItems: "Добавить вещи",
    analyzeOutfit: "Анализ",
    pin: "Закрепить аутфит",
    unpin: "Открепить аутфит",
    nameLabel: "Название аутфита",
    saveAsTitle: "Сохранить образ как",
    renameTitle: "Переименовать образ",
    deleteTitle: "Удалить образ",
    revertTitle: "Откатить изменения",
    deleteConfirmBody:
      "Вы уверены, что хотите удалить этот образ? Это действие нельзя отменить.",
    revertConfirmBody:
      "Отменить текущие несохраненные изменения и вернуть последнюю сохраненную версию образа?",
    deleteConfirm: "Удалить",
    revertConfirm: "Откатить",
    searchPlaceholder: "Поиск аутфитов...",
    searchEmpty: "Аутфиты не найдены.",
    searchPrevious7Days: "Последние 7 дней",
    searchPrevious30Days: "Последние 30 дней",
    searchEarlier: "Ранее",
    loading: "Загружаем образ",
    emptySummary: "Пока нет вещей",
    categoryCount: "{category}: {count}",
    openActions: "Открыть действия с образом",
    selectItem: "Выбрать",
    removeSelected: "Удалить выбранные",
    removeSelectedCount: "Удалить выбранные ({count})",
    removeItemTitle: "Удалить вещь",
    removeSelectedTitle: "Удалить выбранные вещи",
    removeConfirm: "Удалить",
    personalItems: "Личные вещи",
    catalog: "Каталог",
    noneSelected: "Ничего не выбрано",
    personalSelected: "Личные: {count}",
    catalogSelected: "Каталог: {count}",
    totalResults: "Результатов: {count}",
    confirmDelete: "Удалить этот образ? Это действие нельзя отменить.",
    confirmRevert: "Отменить несохраненные изменения в этом образе?",
    confirmRemoveItem: "Удалить эту вещь из образа?",
    confirmRemoveSelected: "Удалить выбранные вещи из образа?",
    itemNotFoundTitle: "Вещь не найдена",
    itemNotFoundDescription: "Эта ссылка в образе больше не находится.",
    openMissingItemActions: "Открыть действия с ненайденной вещью",
    openOutfitActions: "Действия с образом {name}",
    regenerateReport: "Создать отчет заново",
    reportTitle: "Отчет по образу",
    reportOpenMenu: "Открыть действия с отчетом",
    reportGenerating: "Создаем отчет по образу",
    reportOutdated: "Отчет может быть устаревшим",
    reportShowDetails: "Показать детали",
    reportHideDetails: "Скрыть детали",
    reportScores: "Оценки",
    reportStrengths: "Сильные стороны",
    reportIssues: "Проблемы",
    reportIssueSuggestionLabel: "Совет:",
    reportSuggestions: "Советы",
    reportConfidence: "Уверенность",
    reportTemperatureRange: "{min}–{max}°C",
    reportTemperatureFrom: "от {min}°C",
    reportTemperatureUpTo: "до {max}°C",
    reportScoreStyleCoherence: "Стилевая цельность",
    reportScoreColorHarmony: "Цветовая гармония",
    reportScoreSeasonFit: "Соответствие сезону",
    reportScoreFormalityCoherence: "Единая формальность",
    reportScoreOverallCompatibility: "Общая совместимость",
    reportVerdict: {
      valid: "Хорошее сочетание",
      acceptable_with_notes: "Есть замечания",
      incomplete: "Не хватает вещей",
      incoherent: "Нужно доработать",
    },
  },
  wardrobe: {
    title: "Личные вещи",
    allItems: "Все вещи",
    yourOutfits: "Ваши аутфиты",
    searchOutfits: "Поиск аутфитов",
    newOutfit: "Новый аутфит",
    subtitle: "Сохраненные товары и загруженные вещи в одном месте.",
    upload: "Загрузить фото вещи",
    uploadMenu: "Выбрать способ загрузки",
    uploadMenuLabel: "Способы загрузки",
    uploadPhoto: "Загрузить фото",
    uploadUrl: "Загрузить URL изображения",
    analyzePersonalItems: "Анализ",
    regenerateReport: "Создать отчет заново",
    openMenu: "Открыть меню личных вещей",
    filterLabel: "Источник личных вещей",
    loadFailed: "Не удалось загрузить личные вещи.",
    reportLoadFailed: "Не удалось загрузить отчет по личным вещам.",
    reportGenerateFailed: "Не удалось создать отчет по личным вещам.",
    reportLimitActive:
      "Анализ личных вещей уже выполняется. Дождитесь его завершения и попробуйте снова.",
    reportDeleteFailed: "Не удалось удалить отчет по личным вещам.",
    downloadFailed:
      "Не удалось экспортировать личные вещи в PDF. Попробуйте еще раз.",
    uploadFailed: "Не удалось загрузить фото личных вещей. Попробуйте еще раз.",
    urlUploadFailed:
      "Не удалось загрузить URL изображений. Попробуйте еще раз.",
    uploadLimitActive:
      "Уже выполняется слишком много загрузок личных вещей. Дождитесь завершения одной из них и попробуйте снова.",
    saved: "Сохранено в личные вещи.",
    removed: "Удалено из личных вещей.",
    updated: "Личная вещь обновлена.",
    savedBadge: "Сохранено",
    likedBadge: "Лайкнуто",
    failedUploadBadge: "Failed",
    noCategoryBadge: "No category",
    needsReviewBadge: "Требует проверки",
    saveFailed: "Не удалось сохранить в личные вещи. Попробуйте еще раз.",
    saveNotFound: "Этого товара больше нет в каталоге.",
    removeFailed: "Не удалось удалить из личных вещей. Попробуйте еще раз.",
    like: "Лайк",
    removeLike: "Убрать лайк",
    likeFailed: "Не удалось обновить лайк. Попробуйте еще раз.",
    updateFailed: "Не удалось обновить вещь. Попробуйте еще раз.",
    deleteUploaded: "Удалить вещь",
    deleteUploadedConfirmTitle: "Удалить загруженную вещь?",
    deleteUploadedConfirmBody:
      "Загруженная вещь и все ее изображения будут удалены без возможности восстановления.",
    deleteUploadedConfirm: "Удалить",
    removeConfirmTitle: "Удалить из личных вещей?",
    removeConfirmBody:
      "Товар будет удален из личных вещей. Его можно будет сохранить снова позже.",
    removeConfirm: "Удалить",
    emptyTitle: "Пока нет сохраненных вещей",
    emptyBody: "Сохраняйте товары из капсул или позднее загружайте фото вещей.",
    filteredEmptyTitle: "Здесь нет лайкнутых вещей",
    filteredEmptyBody:
      "Лайкайте вещи, чтобы они оставались видимыми в этом фильтре.",
    reportTitle: "Отчет по личным вещам",
    reportOpenMenu: "Открыть действия с отчетом",
    reportGenerating: "Создаем отчет по личным вещам",
    reportOutdated: "Отчет может быть устаревшим",
    reportShowDetails: "Показать детали",
    reportHideDetails: "Скрыть детали",
    reportScores: "Оценки",
    reportOverview: "Обзор",
    reportCoverage: "Покрытие",
    reportOutfitReadiness: "Готовность к аутфитам",
    reportVersatility: "Вариативность",
    reportStyleProfile: "Стилевой профиль",
    reportSeasonality: "Сезонность",
    reportColorAnalysis: "Анализ цвета",
    reportEfficiency: "Эффективность",
    reportStrengths: "Сильные стороны",
    reportIssues: "Проблемы",
    reportIssueSuggestionLabel: "Совет:",
    reportSuggestions: "Советы",
    reportConfidence: "Уверенность",
    reportScoreCoverage: "Покрытие",
    reportScoreOutfitReadiness: "Готовность к аутфитам",
    reportScoreVersatility: "Вариативность",
    reportScoreSeasonality: "Сезонность",
    reportScoreStyleClarity: "Ясность стиля",
    reportScoreColorHarmony: "Цветовая гармония",
    reportScoreEfficiency: "Эффективность",
    reportOverallScore: "Общая оценка",
    reportDetectedCategoryBalance: "Баланс категорий",
    reportDominantStyles: "Доминирующие стили",
    reportDominantSeasons: "Доминирующие сезоны",
    reportDominantFormalityLevels: "Доминирующая формальность",
    reportSummaryTags: "Теги обзора",
    reportRoleTops: "Верх",
    reportRoleBottoms: "Низ",
    reportRoleShoes: "Обувь",
    reportRoleLayers: "Слои",
    reportRoleDresses: "Платья",
    reportRoleAccessories: "Аксессуары",
    reportMissingCategories: "Отсутствующие категории",
    reportWeakCategories: "Слабые категории",
    reportOverrepresentedCategories: "Избыточные категории",
    reportSupportedFormulaTypes: "Поддерживаемые формулы аутфитов",
    reportEstimatedOutfitRange: "Оценочный диапазон аутфитов",
    reportEstimatedOutfitRangeValue:
      "{min}-{max} аутфитов, уверенность: {confidence}",
    reportMainBlockers: "Главные ограничения",
    reportMixAndMatchScore: "Оценка сочетаемости",
    reportRepeatabilityScore: "Оценка повторяемости",
    reportOutfitVariety: "Разнообразие аутфитов",
    reportPrimaryUseModes: "Основные сценарии",
    reportLimitingFactors: "Ограничивающие факторы",
    reportPrimaryStyles: "Основные стили",
    reportStyleCluster: "Стилевой кластер",
    reportFragmentation: "Фрагментация",
    reportItemCount: "{count} вещей",
    reportSeasonSpring: "Весна",
    reportSeasonSummer: "Лето",
    reportSeasonAutumn: "Осень",
    reportSeasonWinter: "Зима",
    reportPrimarySeasons: "Основные сезоны",
    reportWeakSeasons: "Слабые сезоны",
    reportTemperatureBand: "Температурный диапазон",
    reportTemperatureRange: "{min}–{max}°C",
    reportTemperatureFrom: "от {min}°C",
    reportTemperatureUpTo: "до {max}°C",
    reportLayeringSupport: "Поддержка слоев",
    reportWeatherSuitability: "Подходящая погода",
    reportWeatherLimitations: "Погодные ограничения",
    reportPaletteType: "Тип палитры",
    reportBaseColors: "Базовые цвета",
    reportAccentColors: "Акцентные цвета",
    reportContrastLevel: "Уровень контраста",
    reportHarmony: "Гармония",
    reportColorGaps: "Пробелы в цветах",
    reportColorRisks: "Цветовые риски",
    reportRedundancyLevel: "Уровень повторов",
    reportOrphanItemRisk: "Риск вещей-сирот",
    reportNotableRedundancies: "Заметные повторы",
    reportPotentialOrphans: "Потенциальные вещи-сироты",
    reportUnderusedStrengths: "Недоиспользованные сильные стороны",
    reportExpectedImpact: "Ожидаемый эффект",
    reportTargetCategory: "Целевая категория",
    reportReplacementCategory: "Категория замены",
    reportReplacementDescription: "Описание замены",
    reportLowConfidenceAspects: "Аспекты с низкой уверенностью",
    reportAssumptions: "Допущения",
    reportRelatedItems: "Вещи:",
    reportUnnamedItem: "Вещь {id}",
    reportVerdict: {
      excellent: "Отличный набор личных вещей",
      good: "Хороший набор личных вещей",
      usable_with_gaps: "Можно использовать с пробелами",
      unbalanced: "Несбалансированно",
      incomplete: "Не хватает вещей",
      unclear: "Недостаточно ясно",
    },
    filters: {
      all: "Все",
      uploaded: "Загруженные",
      fromCatalog: "Каталог",
      likedOnly: "Только лайкнутые",
    },
    uploadDialog: {
      title: "Загрузить фото личных вещей",
      body: "Используйте отдельное изображение для каждой вещи. Снимите предмет расправленным или аккуратно подвешенным, целиком в кадре, без других вещей, на ровном однотонном фоне.",
      dropzoneLabel: "Выбрать фото личных вещей",
      dropzoneTitle: "Загрузить фото",
      dropzoneHint:
        "Перетащите изображения или нажмите, чтобы выбрать. JPEG, PNG или WebP. До 5 файлов, каждый до 10 MB.",
      mobileDropzoneTitle: "Загрузить фото",
      mobileDropzoneHint:
        "Нажмите, чтобы выбрать. JPEG, PNG или WebP. До 5 файлов, каждый до 10 MB.",
      fileList: "Выбранные файлы",
      selectedSummary: "{count} файлов, {size}",
      removeFile: "Убрать {name}",
      upload: "Загрузить",
      uploadedStatus: "Загружено: {image_count}",
      metadataProcessedStatus: "Метаданные обработаны: {image_count}",
      imageProcessedStatus: "Изображения обработаны: {image_count}",
      failedStatus: "Ошибки: {image_count}",
      tooManyFiles: "Можно загрузить до 5 файлов.",
      invalidType: "Используйте изображения JPEG, PNG или WebP.",
      fileTooLarge: "Каждое изображение должно быть не больше 10 MB.",
    },
    urlUploadDialog: {
      title: "Загрузить URL изображений товаров",
      body: "Добавьте ссылки на коммерческие изображения продукта, где вещь хорошо видна и разложена на плоскости. Каждое подходящее изображение станет загруженной личной вещью.",
      fieldLabel: "URL изображения товара {index}",
      placeholder: "https://example.com/product-image.jpg",
      helperText:
        "Используйте URL изображения продукта, начинающийся с http:// или https://.",
      invalidUrl: "Введите URL, который начинается с http:// или https://.",
      upload: "Загрузить URL изображений",
    },
    imageVersionToggle: {
      label: "Версия изображения загруженной вещи",
      original: "Original",
      ai: "AI",
    },
    uploadedDetail: {
      notSpecified: "Не указано",
      missingRequired: "Чтобы применить изменения, заполните: {items}.",
      fields: {
        name: "Название",
        description: "Описание",
        brand: "Бренд",
      },
      required: {
        name: "название",
        audience: "для кого",
        category: "категорию",
        season: "хотя бы один сезон",
      },
    },
  },
  search: {
    title: "Каталог: Обзор",
    placeholder:
      "Ищите на естественном языке, например: свободная голубая льняная рубашка для летнего офиса",
    cta: "Искать",
    clear: "Очистить поиск",
    productCard: "Карточка товара",
    productDetailsTitle: "Детали товара",
    detailEmpty: "Выберите товар, чтобы посмотреть подробности.",
    detailLoading: "Загружаем детали товара",
    empty: "По этому запросу ничего не найдено.",
    untitled: "Товар без названия",
    noBrand: "Бренд не указан",
    notImportant: "Не принципиально",
    all: "Все",
    back: "Назад",
    productActions: "Действия с товаром",
    openProductPage: "Открыть страницу товара",
    yes: "Да",
    no: "Нет",
    resultsCount: "Результатов: {count}",
    priceMin: "Мин: {value}",
    priceMax: "Макс: {value}",
    filters: {
      query: "Запрос: {query}",
      likedItems: "Только лайкнутые",
      likedItemsOnly: "Только лайкнутые",
      brand: "Бренд",
      price: "Цена",
      min: "Мин",
      max: "Макс",
      category: "Категория",
      exactColor: "Подбор по оттенку",
      exactColorChip: "Подбор по оттенку: {color} · {range}",
      exactColorToggle: "Включить подбор по оттенку",
      exactColorHint: "Ищет вещи с близким заметным оттенком.",
      exactColorPicker: "Выбрать оттенок",
      exactColorHex: "Цвет HEX",
      exactColorInvalid: "Введите 6-значный HEX-код, например #808080.",
      exactColorRange: "Диапазон совпадения",
      exactColorRangeAria: "Диапазон подбора по оттенку",
      exactColorRangeValue: "Диапазон подбора: {value}",
      exactColorRangeCloser: "Ближе",
      exactColorRangeClosest: "Максимально близкий",
      exactColorRangeClose: "Близкий",
      exactColorRangeBalanced: "Сбалансированный",
      exactColorRangeBroad: "Широкий",
      exactColorRangeBroadest: "Максимально широкий",
      exactColorRangeBroader: "Шире",
      exactColorRangeHint:
        "Более широкий диапазон включает больше вариаций цвета.",
      silhouette: "Силуэт",
      fit: "Посадка",
      closureType: "Тип застежки",
    },
    fields: {
      price: "Цена",
      availability: "Наличие",
      audience: "Для кого",
      season: "Сезон",
      formalityLevel: "Формальность",
      style: "Стиль",
      occasions: "Ситуации",
      color: "Цвет",
      pattern: "Паттерн",
      finish: "Фактура",
      neutral: "Нейтральный",
      composition: "Состав",
      silhouette: "Силуэт",
      fit: "Посадка",
      closureType: "Тип застежки",
    },
  },
  statistics: {
    title: "Каталог: Статистика",
    total: "Всего",
    totalHint:
      "Графики считаются по полному набору результатов с учетом всех текущих фильтров.",
    activeFilters: "Активные фильтры",
    empty: "Для текущих фильтров нет данных.",
    noActiveFilters: "Активных фильтров нет.",
    refresh: "Обновить",
    chartHint:
      "Нажмите на сегмент, чтобы наложить или снять соответствующий фильтр.",
    priceHint: "Нажмите на столбец, чтобы сузить поиск до этого диапазона цен.",
    charts: {
      formalityLevel: "Формальность",
      style: "Стиль",
    },
  },
  errors: {
    generic: "Что-то пошло не так. Попробуйте еще раз.",
    invalidEmail: "Проверьте формат email.",
    cooldown: "Подождите минуту перед повторной отправкой.",
    rateLimit: "Слишком много попыток. Попробуйте позже.",
    expired: "Код истек. Запросите новый.",
    maxAttempts: "Слишком много неверных попыток.",
    invalidCode: "Неверный код. Проверьте письмо.",
    profileExists: "Профиль уже существует.",
    profileNotFound: "Профиль не найден.",
    invalidPayload: "Выберите хотя бы один вариант.",
    tooManyRequests:
      "Слишком много запросов. Подождите немного и попробуйте снова.",
    tooManyActiveJobs:
      "Уже выполняется несколько задач. Дождитесь завершения одной из них и попробуйте снова.",
    tooManyMcpSessions:
      "Открыто слишком много сессий коннектора. Закройте одну из них и попробуйте снова.",
    generationLimitActive:
      "Уже выполняется несколько задач генерации. Дождитесь завершения одной из них и попробуйте снова.",
    invalidGoogleToken: "Не удалось войти через Google. Попробуйте снова.",
    googleAuthNotConfigured: "Вход через Google не настроен на сервере.",
    passkeyNotSupported:
      "Passkeys не поддерживаются в этом браузере или на этом устройстве.",
    passkeySetupFailed: "Не удалось настроить passkey.",
    passkeyLoginFailed: "Не удалось войти с passkey.",
    downloadFailed: "Не удалось сгенерировать PDF капсулы. Попробуйте еще раз.",
    regenerateAllFailed:
      "Не удалось обновить капсулу. Предыдущая версия восстановлена.",
    regenerateSelectedFailed:
      "Не удалось обновить выбранные карточки. Попробуйте еще раз.",
    outfitReportGenerateFailed: "Не удалось создать отчет по образу.",
    capsuleReportGenerateFailed: "Не удалось создать отчет по капсуле.",
    capsuleContainsPersonalItems:
      "Капсулами с личными загруженными вещами нельзя делиться.",
    capsuleNotShareable: "В капсуле нет содержимого, которым можно поделиться.",
    sharedCapsuleUnavailable:
      "Эта общая капсула недоступна или срок ссылки истек.",
  },
  dialogs: {
    signOutTitle: "Выйти",
    signOutBody: "Вы уверены, что хотите выйти?",
    signOutCancel: "Отмена",
    signOutConfirm: "Выйти",
  },
};

export default ru;
